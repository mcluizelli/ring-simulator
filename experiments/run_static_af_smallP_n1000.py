"""
The remaining mixed-n ring sizes in the controller dataset: P in {4, 8, 32}.

v5.7 fixed P=64 and v5.8 fixed P=16; the static sweep also carries P in {4, 8, 32},
still at n=100 from v5.3. None of them is plotted or quoted, but v5.9 is named
'all_n1000' and that name has to be literally true.

Identical to run_static_af_p16_n1000.py in every constant; only the ring size varies.

Usage:  python experiments/run_static_af_smallP_n1000.py [n=1000] [workers]
Output: results/v5.9_static_af_smallP_n1000/{results.csv, summary.csv}
"""
...
OUT_DIR = _RING_ROOT / "results" / "v5.9_static_af_smallP_n1000"
_TOPO = None
...
def _work(args):
    ...
    return {"run": run_idx, "P": P, "k": k, "affected_fraction": af, "completion_time_s": t}
...
def main():
    ...
    tasks = [(r, k, af, P) for P in P_LIST for af in AF_LIST for r in range(n) for k in SWEEP_K]
    print(f"static af-sweep P={P_LIST} af={AF_LIST} k={SWEEP_K} n={n} on {nw} workers "
    ...
    rows.sort(key=lambda r: (r["P"], r["affected_fraction"], r["run"], r["k"]))
    with open(OUT_DIR / "results.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["run", "P", "k", "affected_fraction", "completion_time_s"])
        w.writeheader(); w.writerows(rows)

    T = {(r["P"], r["affected_fraction"], r["run"], r["k"]): r["completion_time_s"] for r in rows}
    summ = []
    for P in P_LIST:
      for af in AF_LIST:
        runs = sorted({r["run"] for r in rows if r["affected_fraction"] == af and r["P"] == P})
        for k in SWEEP_K:
            sp = [T[(P, af, r, 1)] / T[(P, af, r, k)] for r in runs if T.get((P, af, r, k))]
            m, lo, hi = boot_ci(sp)
            summ.append({"ring_size": P, "k": k, "affected_fraction": af, "n_seeds": len(sp),
                         "speedup": m, "speedup_ci_low": lo, "speedup_ci_high": hi,
                         "total_qps": k * P})
            print(f"  P={P} af={af} k={k}: {m:.3f} [{lo:.3f},{hi:.3f}]  (n={len(sp)})", flush=True)
