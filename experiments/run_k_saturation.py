"""
C.3.7 stage 2 — k-SATURATION sweep: extend k to {1,2,4,8,16,32} to locate the
speed-up plateau per fabric (the v9.0 sweep stopped at k=8 while the
oversubscribed curves were still rising — k* cannot be read off unsaturated
data).

Same methodology and guards as run_topology_sweep.py (v9.0):
  per-seed-paired speed-up + bootstrap 95% CI; randomised ring placement per
  seed (matched across the 3-tier fabrics); sim never beats the optimal
  proportional-split bound; k=1 equal==optimal; mechanism counters recorded
  (top_used, max_top_contention, equal_split_gap).

Purpose-specific additions:
  - k up to 32 (past the analytic k_e half-efficiency point of 2-tier, M=8);
  - per-cell speed-up MIN (over-spreading harm guard: does high k ever hurt?).

Usage:  python experiments/run_k_saturation.py [n_seeds] [n_workers]
"""
...
P_LIST = [16, 64]
SEED_BASE = 9000          # same base as v9.0 -> same placements, comparable
FABRICS = {
    "3tier_nb":  (3, 1.0),
    "3tier_os2": (3, 2.0),
    "3tier_os4": (3, 4.0),
...
OUT_DIR = _BootPath("results/v10.0_k_saturation_2026-07-02")
...
def main():
    ...
    seeds = list(range(n))
    print(f"C.3.7 k-saturation sweep: {list(FABRICS)} x k{SWEEP_K} x P{P_LIST} x {n} seeds, {nw} workers\n", flush=True)
